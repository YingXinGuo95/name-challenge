...
export function CompletionScreen({
  ...
  useEffect(() => {
    ...
    const frame = () => {
      confetti({
        ...
        spread: 55,
        origin: { x: 0, y: 0.7 },
        colors: ["#FFE066", "#6CB4EE", "#9333EA", "#5CC9C7", "#FFD700"],
      });
      confetti({
        particleCount: 3,
        angle: 120,
  ...
  function buildShareText(): string {
    if (submissionResult) {
      return `🎬 I ranked #${submissionResult.rank} — named ${targetCount} movies in ${elapsed}!\n\nCan you beat my score? Play Name 100 Movies Challenge!`;
    }
    return `🎬 I just named ${targetCount} movies in ${elapsed}!\n\nCan you beat my score? Play Name 100 Movies Challenge!`;
  }
  ...
  const handleCopyImage = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        a.href = url;
        a.download = "name-100-movies-result.png";
        a.click();
  ...
  if (screenState === "celebration") {
    return (
      <div className="flex flex-col items-center gap-8">
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-sm text-gray-600">
              You named{" "}
              <span className="font-bold text-[#9333EA]">{targetCount} famous movies</span>.
            </p>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            {!alreadySubmitted && isCheckingRank && (
              <button
                disabled
                className="inline-flex items-center gap-2 rounded-full bg-[#9333EA]/50 px-6 py-3 text-sm font-bold text-[#2D2D2D]/50"
              >
            ...
            {!alreadySubmitted && !isCheckingRank && currentRank !== null && currentRank <= 100 && (
              <div className="flex flex-col items-center gap-2">
                <p className="text-sm font-bold text-[#2D2D2D]">
                  🎉 You&apos;re in the{" "}
                  <span className="text-base font-extrabold text-[#9333EA]">
                    top {currentRank}
                  </span>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-[#9333EA] px-6 py-3 text-sm font-bold text-white transition-transform hover:scale-105"
                >
        ...
        {alreadySubmitted && (
          <div className="rounded-full border-[2.5px] border-[#9333EA] bg-[#9333EA]/10 px-5 py-2 text-center">
            <p className="text-sm font-bold text-[#2D2D2D]">
              You ranked{" "}
              <span className="text-base font-extrabold text-[#9333EA]">
    ...
    return (
      <div className="flex w-full max-w-md flex-col items-center gap-8">
        ...
        >
          ...
          {submitError && (
            <div
              className="retro-card flex w-full items-center gap-2 bg-[#9333EA]/20 px-4 py-3 text-sm font-bold text-[#2D2D2D]"
              role="alert"
            >
