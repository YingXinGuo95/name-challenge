    ...
    async (name: string) => {
      ...
      try {
        const res = await fetch(
          `/api/validate-name?q=${encodeURIComponent(name)}&gender=male`
        );
  ...
  return (
    <div className="flex flex-1 flex-col items-center gap-8 px-4 py-10 sm:py-14 lg:grid lg:grid-cols-[1fr_auto_1fr] lg:items-start lg:gap-0 lg:px-8">
      ...
      <div className="flex flex-col items-center gap-8 lg:col-start-2 lg:max-w-xl">
        ...
        <div className="space-y-4 text-center">
          ...
          <div className="flex items-center justify-center gap-2" aria-hidden="true">
            <Sparkles className="h-4 w-4 text-[#5B9BD5]/60" />
          ...
          {/* Title with "Men" highlighted */}
          <h1 className="text-4xl font-extrabold uppercase tracking-tight text-[#2D2D2D] sm:text-5xl">
            Name 100{" "}
            <span className="relative inline-block">
              <span className="relative z-10 text-[#5B9BD5]">Men</span>
              <span
                className="absolute bottom-0 left-0 z-0 h-[10px] w-full rounded-full bg-[#5B9BD5]/20"
                aria-hidden="true"
              />
            </span>
            {" "}Challenge
          ...
          <p className="mx-auto max-w-md text-sm font-medium leading-relaxed text-muted-foreground">
            ...
            <strong className="font-extrabold uppercase text-[#5B9BD5]">
              remarkable men
            </strong>{" "}
            across history — scientists, artists, leaders, and athletes.
        ...
        <MobileLeaderboardDialog challengeSlug="name-100-men" />
        ...
        {error && (
          <div
            className="retro-card flex w-full max-w-xl items-center gap-2 bg-[#5B9BD5]/30 px-5 py-3 text-sm font-bold text-[#2D2D2D]"
            role="alert"
          >
      ...
      <div className="hidden lg:block lg:sticky lg:top-24 lg:col-start-3 lg:justify-self-end lg:pr-0">
        <LiveLeaderboard challengeSlug="name-100-men" />
      </div>
    </div>
  );
}
