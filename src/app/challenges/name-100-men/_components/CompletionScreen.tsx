...
export function CompletionScreen({
  ...
  useEffect(() => {
    ...
    const frame = () => {
      confetti({
        ...
        spread: 55,
        origin: { x: 0, y: 0.7 },
        colors: ["#FFE066", "#6CB4EE", "#5B9BD5", "#5CC9C7", "#FFD700"],
      });
      confetti({
        particleCount: 3,
        angle: 120,
  ...
  function buildShareText(): string {
    if (submissionResult) {
      return `🏆 I ranked #${submissionResult.rank} — named ${targetCount} famous men in ${elapsed}!\n\nCan you beat my score? Play Name 100 Men Challenge!`;
    }
    return `🏆 I just named ${targetCount} famous men in ${elapsed}!\n\nCan you beat my score? Play Name 100 Men Challenge!`;
  }
  ...
  const handleCopyImage = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        a.href = url;
        a.download = "name-100-men-result.png";
        a.click();
  ...
  if (screenState === "celebration") {
    return (
      <div className="flex flex-col items-center gap-8">
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-sm text-gray-600">
              You named{" "}
              <span className="font-bold text-[#5B9BD5]">{targetCount} famous men</span>.
            </p>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            {!alreadySubmitted && isCheckingRank && (
              <button
                disabled
                className="inline-flex items-center gap-2 rounded-full bg-[#5B9BD5]/50 px-6 py-3 text-sm font-bold text-[#2D2D2D]/50"
              >
            ...
            {!alreadySubmitted && !isCheckingRank && currentRank !== null && currentRank <= 100 && (
              <div className="flex flex-col items-center gap-2">
                <p className="text-sm font-bold text-[#2D2D2D]">
                  🎉 You&apos;re in the{" "}
                  <span className="text-base font-extrabold text-[#5B9BD5]">
                    top {currentRank}
                  </span>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-[#5B9BD5] px-6 py-3 text-sm font-bold text-[#2D2D2D] transition-transform hover:scale-105"
                >
        ...
        {alreadySubmitted && (
          <div className="rounded-full border-[2.5px] border-[#5B9BD5] bg-[#5B9BD5]/10 px-5 py-2 text-center">
            <p className="text-sm font-bold text-[#2D2D2D]">
              You ranked{" "}
              <span className="text-base font-extrabold text-[#5B9BD5]">
    ...
    return (
      <div className="flex w-full max-w-md flex-col items-center gap-8">
        ...
        >
          ...
          {submitError && (
            <div
              className="retro-card flex w-full items-center gap-2 bg-[#5B9BD5]/30 px-4 py-3 text-sm font-bold text-[#2D2D2D]"
              role="alert"
            >
